...
def _managed_persistence_enabled() -> bool:
    ...
    Checks the env var first (``CAMOFOX_MANAGED_PERSISTENCE``), then falls
    back to ``browser.camofox.managed_persistence`` in config.yaml.
    """
    env_val = os.getenv("CAMOFOX_MANAGED_PERSISTENCE", "").lower()
